...
help: ## 사용 가능한 Make 명령어 목록 표시
	...
	@echo -e "\033[1;34m║              Kubernetes 클러스터 자동화 배포 도구 (Ansible)                ║\033[0m"
	@echo -e "\033[1;34m╚══════════════════════════════════════════════════════════════════════════════╝\033[0m"
	@echo ""
	@echo -e "\033[1m사용법:\033[0m"
	@echo -e "  make \033[36m<target>\033[0m"
	@echo ""
	@echo -e "\033[1m빠른 시작 예제:\033[0m"
	@echo -e "  \033[32mmake ping\033[0m              # 호스트 연결 테스트"
	@echo -e "  \033[32mmake install\033[0m           # 전체 클러스터 설치"
	@echo -e "  \033[32mmake check-cluster\033[0m     # 클러스터 상태 확인"
	@echo ""
	@echo -e "\033[1m신규 기능 사용 (Runway 2.0+):\033[0m"
	@echo -e "  \033[32mmake configure-runway\033[0m  # Runway 2.0+ 필수 설정 (레지스트리 미러)"
	@echo -e "  \033[32mmake configure-gpu-full\033[0m # GPU 전체 설정 (드라이버+레이블)"
	@echo -e "  \033[32mmake tag-oidc-apiserver\033[0m # OIDC 인증 설정"
	@echo -e "  \033[32mmake tag-label-gpu-nodes\033[0m # GPU 노드 레이블링"
	@echo ""
	@awk 'BEGIN {FS = ":.*##"; printf ""} /^[a-zA-Z_-]+:.*?##/ { printf "  \033[36m%-28s\033[0m %s\n", $$1, $$2 } /^##@/ { printf "\n\033[1;33m%s\033[0m\n", substr($$0, 5) } ' $(MAKEFILE_LIST)
	@echo ""
	@echo -e "\033[1m유용한 팁:\033[0m"
	@echo -e "  • 특정 호스트 명령: \033[36mmake cmd-host HOST=\"master1\" CMD=\"uptime\"\033[0m"
	@echo -e "  • 모든 호스트 명령:  \033[36mmake cmd-all CMD=\"df -h\"\033[0m"
	@echo -e "  • 모든 신규 설정:    \033[36mmake apply-all-configs\033[0m (확인 필요)"
	@echo -e "  • GPU 전체 설정:     \033[36mmake configure-gpu-full\033[0m"
	@echo -e "  • NVIDIA 상태 확인:  \033[36mmake check-nvidia-gpu && make check-nvidia-driver\033[0m"
	@echo ""
	@echo -e "\033[1m더 많은 정보:\033[0m"
...
tag-set-hostname: ## inventory_hostname으로 시스템 호스트 이름 설정
	@echo "==> 시스템 호스트 이름 설정 중..."
	ansible-playbook -i $(INVENTORY) $(PLAYBOOK) --tags set-hostname

tag-etc-hosts: ## 인벤토리 기반 /etc/hosts 파일 구성
	@echo "==> /etc/hosts 파일 구성 중..."
	ansible-playbook -i $(INVENTORY) $(PLAYBOOK) --tags etc-hosts
...
httpd-repo-stop: ## httpd 서비스 중지 [root 필요]
	@sudo ./scripts/manage-rhel-repo.sh httpd-stop

httpd-repo-restart: ## httpd 서비스 재시작 [root 필요]
	@sudo ./scripts/manage-rhel-repo.sh httpd-restart
